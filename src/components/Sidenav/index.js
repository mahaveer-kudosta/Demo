...
function Sidenav({ color, brand, brandName, routes, ...rest }) {
  ...
  const renderRoutes = routes.map(({ type, name, icon, title, noCollapse, key, href, route }) => {
    ...
    if (type === "collapse") 
      {
        ...
        {
          returnValue = (
            <Link key={key} sx={{ textDecoration: "none" }}
              onClick={(e) => {
                e.preventDefault();
                logout();
              }}
    ...
    {
      returnValue = (
        <MDTypography key={key} color={textColor} display="block" variant="caption" fontWeight="bold" 
        textTransform="uppercase" pl={3} mt={2} mb={1} ml={1}>
          {title}
        </MDTypography>
      );
    } 
